...
static GstElement* request_aux_sender(GstElement*, guint);
static GstElement* request_aux_receiver(GstElement*, guint);
 ...
 */
static GstElement* request_aux_sender(GstElement* rtp_bin, guint connected_id)
{
  GstElement *rtx, *bin;
  GstPad *pad;
  gchar *name;
  ...
  name = g_strdup_printf("src_%u", connected_id);
  gst_element_add_pad(bin, gst_ghost_pad_new(name, pad));
  g_free(name);
  gst_object_unref(pad);

  pad = gst_element_get_static_pad(rtx, "sink");
  name = g_strdup_printf("sink_%u", connected_id);
  gst_element_add_pad(bin, gst_ghost_pad_new(name, pad));
  g_free(name);
  gst_object_unref(pad);
...
static GstElement* request_aux_receiver(GstElement* rtp_bin, guint connected_id)
{
  GstElement *rtx, *bin;
  GstPad *pad;
  gchar *name;
 ...
 */
static GstCaps* request_pt_map(GstElement* source, guint connected_id,
